...
private:
  ...
  bool inBorderResized(const cv::Point2f &pt);
  ...
  nms_process(const vector<cv::Point2f> &pts, const vector<int> &sorted_idx,
              float dist_thresh);

  vector<cv::Point2f> pts_ori_to_resized(const vector<cv::Point2f> &ori_pts);
  cv::Point2f pt_ori_to_resized(const cv::Point2f &ori_pt);
  vector<cv::Point2f>
  pts_resized_to_ori(const vector<cv::Point2f> &resized_pts);
  cv::Point2f pt_resized_to_ori(const cv::Point2f &resized_pt);
