...
bool FeatureTracker::inBorderResized(const cv::Point2f &pt) {
  int BORDER_SIZE = feature_tracker_config.borders;
  int img_x = cvRound(pt.x);
  int img_y = cvRound(pt.y);
  return BORDER_SIZE <= img_x &&
         img_x < feature_tracker_config.resized_width - BORDER_SIZE &&
         BORDER_SIZE <= img_y &&
         img_y < feature_tracker_config.resized_height - BORDER_SIZE;
}

//特征点缩放：原始图像大小->网络输入大小
vector<cv::Point2f>
FeatureTracker::pts_ori_to_resized(const vector<cv::Point2f> &ori_pts) {
...
cv::Point2f FeatureTracker::pt_ori_to_resized(const cv::Point2f &ori_pt) {
  double w_scale =
      feature_tracker_config.resized_width / (double)feature_tracker_config.col;
  double h_scale = feature_tracker_config.resized_height /
                   (double)feature_tracker_config.row;
  return cv::Point2f(ori_pt.x * w_scale, ori_pt.y * h_scale);
}

//特征点缩放：网络输入大小->原始图像大小
vector<cv::Point2f>
FeatureTracker::pts_resized_to_ori(const vector<cv::Point2f> &resized_pts) {
...
cv::Point2f FeatureTracker::pt_resized_to_ori(const cv::Point2f &resized_pt) {
  double w_scale =
      (double)feature_tracker_config.col / feature_tracker_config.resized_width;
  double h_scale = (double)feature_tracker_config.row /
                   feature_tracker_config.resized_height;
  return cv::Point2f(resized_pt.x * w_scale, resized_pt.y * h_scale);
}
...
vector<cv::Point2f>
FeatureTracker::ptsVelocity(vector<int> &_cur_ids,
                            vector<cv::Point2f> &_cur_un_pts,
...
//基于网格划分实现nms
vector<std::pair<int, cv::Point2f>>
FeatureTracker::nms_process(const vector<cv::Point2f> &pts,
                            const vector<int> &sorted_idx, float dist_thresh) {
...
void FeatureTracker::extractKeyPoints(vector<cv::Point2f> &new_pts,
  ...
  for (int i = 0; i < heat_map_size; ++i) {
    if (*(cur_heatmap + i) < response_thresh)
      continue;
...
void FeatureTracker::extractSquareROIPtsDesc(const cv::Point2f &ori_pt,
  ...
  // 将原始坐标转换为resized坐标
  cv::Point2f resized_pt = pt_ori_to_resized(ori_pt);
  int ori_x = std::floor(resized_pt.x);
  int ori_y = std::floor(resized_pt.y);
  ...
  for (int i = ori_x - half_len; i <= ori_x + half_len; i++) {
    for (int j = ori_y - half_len; j <= ori_y + half_len; j++) {
      ...
      if (cur_heatmap[j * w + i] > track_assist_args.roi_pts_threshold)
        resized_pts.emplace_back(float(i), float(j));
    }
  }
  // extract desc
  extractDescriptors(resized_pts, descs);
  // recover the scale: 从resized坐标转换回原始坐标
  pts = pts_resized_to_ori(resized_pts);
}
...
void FeatureTracker::extractROIFeatsWithNMS(const cv::Point2f &ori_pt,
  ...
  for (auto &[idx, pt] : id_pts_reserved) {
    ...
    pts.push_back(pt_resized_to_ori(pt));
...
void FeatureTracker::checkAndExtractCurFlow(
  ...
  for (size_t i = 0; i < prev_pts.size(); i++) {
    ...
    DescV cur_desc_i = extractSingleDesc(pt_ori_to_resized(cur_pts[i]));
...
void FeatureTracker::retrackThroughDescMatch(
  ...
  ROS_ASSERT((prev_pts.size() == cur_pts.size()) &&
             (prev_pts.size() == cur_desc.size()) &&
...
void FeatureTracker::track_img_use_opticalflow(double _cur_time,
  ...
  if (!n_pts.empty() && feature_tracker_config.use_retrack) {
    ...
    auto n_pts_resized = pts_ori_to_resized(n_pts);
    extractDescriptors(n_pts_resized, new_pts_desc);
...
void FeatureTracker::track_img(double _cur_time, const cv::Mat &_img,
  ...
  if (prev_pts.size() > 0) {
    ...
    ROS_DEBUG("separate unmatched points cost %f ms, matched_counts is %d, "
...
cv::Mat FeatureTracker::getOriImage() { return cur_img; }
...
void FeatureTracker::etractLoopFeatsAndPub(ros::Publisher &pub_msg) {
  ...
  loop_local_pts = pts_resized_to_ori(loop_local_pts);
