...
void LineFeatureTracker::undistortLineOnlyUseK(const vector<KeyLine> &key_lsd,
  ...
  for (auto &line : key_lsd) {
    ...
    pts_s << line.getStartPoint().x, line.getStartPoint().y;
    pts_e << line.getEndPoint().x, line.getEndPoint().y;
    un_pts_s << (pts_s.x() - cx) / fx, (pts_s.y() - cy) / fy, 1.0;
    un_pts_e << (pts_e.x() - cx) / fx, (pts_e.y() - cy) / fy, 1.0;
    line_undist.emplace_back(un_pts_s.x(), un_pts_s.y(), un_pts_e.x(),
                             un_pts_e.y());
  }
}
...
// NMS实现
vector<int>
LineFeatureTracker::lineNMSProcess(const vector<KeyLine> &vecTracked,
...
//划分出垂直线段
vector<LineType>
LineFeatureTracker::lineClassify(const vector<Vector4d> &key_lsd,
...
void LineFeatureTracker::readImage(double _cur_time, const cv::Mat &_img) {
  ...
  if (line_tracker_config.undistort_image == 1) {
    cv::remap(img, img, undist_map1, undist_map2, cv::INTER_LINEAR);
  }
  // equalize
  ...
  if (prevFrame->keyLsd.size() > 0) {
    ...
    if (line_tracker_config.detect_vertical_at_front) {
      ...
      if (line_tracker_config.undistort_image == 1) {
        undistortLineOnlyUseK(vecLineNMS, lineNMSUndist);
      } else {
  ...
  // undistort
  if (line_tracker_config.undistort_image == 1) {
    undistortLineOnlyUseK(curFrame->keyLsd, curFrame->lineSpEpUndist);
  } else {
...
void LineFeatureTracker::DrawLine() {
  ...
  for (int i = 0; i < curFrame->lineID.size(); i++) {
    ...
    cv::line(rgba_image, curFrame->keyLsd[i].getStartPoint(),
             ...
             cv::LINE_AA); // cv::Scalar(255 * (1 - len), 0, 255 * len)
  }
