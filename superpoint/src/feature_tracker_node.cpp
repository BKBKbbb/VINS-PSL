...
ros::Publisher pub_img, pub_match, pub_line_img, pub_pt_line_img, pub_xfeats;
...
void pubTrackImage(ros::Publisher &pub, const cv::Mat &imgTrack,
                   const double t) {
...
cv::Mat DrawPtsAndLines(const cv::Mat &image, const FeatureTracker &pts_tracker,
                        const LineFeatureTracker &lines_tracker) {
  cv::Mat rgba_image;
  cv::cvtColor(image, rgba_image, cv::COLOR_BGR2BGRA);
  // draw points
  for (int i = 0; i < pts_tracker.cur_pts.size(); i++) {
    cv::circle(rgba_image, pts_tracker.cur_pts[i], 2, cv::Scalar(0, 0, 255), 2);
  }
  // draw lines
  for (int i = 0; i < lines_tracker.curFrame->lineID.size(); i++) {
    cv::line(rgba_image, lines_tracker.curFrame->keyLsd[i].getStartPoint(),
             lines_tracker.curFrame->keyLsd[i].getEndPoint(),
             cv::Scalar(255, 0, 0), 2);
  }
  ...
  while (1) {
    ...
    if (pub_this_frame) {
      ...
      if (tracker.feature_tracker_config.show_track) {
        ...
        if (!track_res.empty())
          pubTrackImage(pub_match, track_res, header.stamp.toSec());

        cv::Mat line_track_res = line_tracker.getTrackImage();
        if (!line_track_res.empty())
          pubTrackImage(pub_line_img, line_track_res, header.stamp.toSec());
        // draw pts and lines in one graph
        if (1) {
          cv::Mat ori_image = tracker.getOriImage();
          cv::Mat pts_lines_img =
              DrawPtsAndLines(ori_image, tracker, line_tracker);
          pubTrackImage(pub_pt_line_img, pts_lines_img, header.stamp.toSec());
        }
      }
    }
...
int main(int argc, char **argv) {
  ...
  pub_pt_line_img =
      nh.advertise<sensor_msgs::Image>("/feature_tracker/pt_line_img", 1000);
  pub_xfeats =
