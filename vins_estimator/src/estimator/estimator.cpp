...
void Estimator::setParameter() {
  ...
  ProjectionTwoFrameOneCamFactor::sqrt_info =
      FOCAL_LENGTH / 3 * Matrix2d::Identity(); // 1.5
  ProjectionTwoFrameTwoCamFactor::sqrt_info =
      FOCAL_LENGTH / 3 * Matrix2d::Identity();
  ...
  m_camera =
      CameraFactory::instance()->generateCameraFromYamlFile(CAM_NAMES[0]);
...
void Estimator::processMeasurements() {
  while (1) {
    ...
    if (!featureBuf.empty()) {
      ...
      if (USE_STRUCT_LINE == 2)
        processImageWithPointsAndStructLines(feature.second, feature.first);
      else if (USE_STRUCT_LINE == 1)
        processImage(feature.second, feature.first);
      else
        processImageOnlyPts(feature.second, feature.first);
      prevTime = curTime;
      ...
      if (PUB_IMAGE_AT_BACKEND) {
        ...
        visualizeStructLinePartition(feature.first, feature.second.second,
                                     local_mht);
...
void Estimator::processImageOnlyPts(
    const pair<map<int, vector<pair<Eigen::Matrix<double, 64, 1>,
                                    Eigen::Matrix<double, 7, 1>>>>,
               map<int, pair<vector<uchar>, Eigen::Matrix<double, 8, 1>>>>
        &image,
...
void Estimator::processImageWithPointsAndStructLines(
  ...
  if (solver_flag == INITIAL) {
    ...
    if (STEREO && USE_IMU) {
      ...
      // RANSAC
      // extend the number of new_other_lines with tracked_h_lines for
      // better peoformance of RANSAC
...
void Estimator::vector2double() {
  ...
  if (USE_STRUCT_LINE == 2) {
    ...
    para_Local_MHT[0][0] = local_mht;
             ...
             (enable_line_opti || enable_triang_opti_only)) {
...
void Estimator::optimization() {
  ...
  } else if (USE_STRUCT_LINE == 1 && enable_line_opti) {
    int line_m_cnt = 0;
    int line_index = -1;
  ...
  if (marginalization_flag == MARGIN_OLD) {
    ...
    if (USE_STRUCT_LINE == 2 && ENABLE_STRUCT_LINE_MARGIN) {
      int line_index = -1;
      for (auto &it_per_id : struct_line_manager.struct_line_features) {
        it_per_id.used_num = it_per_id.line_feature_per_frame.size();
        if (!struct_line_manager.isLineUsable(it_per_id))
...
void Estimator::slideWindowOld() {
  ...
  if (USE_STRUCT_LINE == 1)
    line_manager.removeBack();
  else if (USE_STRUCT_LINE == 2) {
    if (mht_state == UPDATING)
...
pair<bool, double>
Estimator::recognizeMHTUsingRANSAC(int frame_count,
                                   const vector<LineFeatsPtr> &cur_lines_all) {
...
void Estimator::calAssociaPtsForLines(
  ...
  for (auto &line : lines) {
    ...
    double max_ly = ly2;
    if (lx1 > lx2)
      std::swap(min_lx, max_lx);
    if (ly1 > ly2)
      std::swap(min_ly, max_ly);
    // cal distance
...
void Estimator::visualizeStructLinePartition(
    double cur_header,
  ...
  cv::Mat image_ori;
  {
    ...
    image_ori = getImageFromMsg(img0_buf.front());
    img0_buf.pop();
  }

  if (image_ori.empty() || cur_lines.empty()) {
    return;
  }

  // 图像去畸变
  cv::Mat image;
  cv::remap(image_ori, image, undist_map1, undist_map2, cv::INTER_LINEAR);

  // draw a string
  // text1
  string text1;
  if (mht_state == HOLD) {
    text1.append("LMW x-orientation: ");
    double local_mht_d = local_mht / M_PI * 180.;
    text1.append(std::to_string(local_mht_d));
    text1.append(" deg");
  } else {
  ...
  int margin = 10;
  int x1 = image.cols - text1Size.width - margin;
  int y1 = image.rows - margin;
  cv::Point textOrg1(x1, y1);
  ...
  // 计算三个方向的消失点（在图像坐标系中）
  Eigen::Vector3d vp_x_world(cos(mht_angle), sin(mht_angle), 0);
  Eigen::Vector3d vp_y_world(-sin(mht_angle), cos(mht_angle), 0);
  Eigen::Vector3d vp_z_world(0, 0, 1);

  // 使用当前帧的姿态将消失点转换到相机坐标系
  int cur_frame = frame_count;
  Eigen::Matrix3d R_wc = Rs[frame_count] * ric[0];
  Eigen::Matrix3d R_cw = R_wc.transpose();
  Eigen::Vector3d vp_x_cam = R_cw * vp_x_world;
  Eigen::Vector3d vp_y_cam = R_cw * vp_y_world;
  Eigen::Vector3d vp_z_cam = R_cw * vp_z_world;

  // 投影到归一化平面
  auto projectVPToNormalizedPlane =
      [&](const Eigen::Vector3d &vp_cam) -> Eigen::Vector2d {
    if (fabs(vp_cam.z()) < 1e-6) {
      // 消失点在无穷远处，返回归一化平面的方向
      return Eigen::Vector2d(vp_cam.x(), vp_cam.y()).normalized() *
             1000.0; // 大的数表示方向
    }
    return Eigen::Vector2d(vp_cam.x() / vp_cam.z(), vp_cam.y() / vp_cam.z());
  };

  Eigen::Vector2d vp_x_norm = projectVPToNormalizedPlane(vp_x_cam);
  Eigen::Vector2d vp_y_norm = projectVPToNormalizedPlane(vp_y_cam);
  Eigen::Vector2d vp_z_norm = projectVPToNormalizedPlane(vp_z_cam);

  // 归一化平面到像素坐标的转换函数
  auto normToPixel = [&](const Eigen::Vector2d &norm_pt,
                         Eigen::Vector2d &pixel_pt) {
    if (UNDISTORT_IMAGE == 1) {
      float fx = K_.at<float>(0, 0);
      float fy = K_.at<float>(1, 1);
      float cx = K_.at<float>(0, 2);
      float cy = K_.at<float>(1, 2);
      pixel_pt(0) = norm_pt(0) * fx + cx;
      pixel_pt(1) = norm_pt(1) * fy + cy;
    } else {
      m_camera->undistToPlane(norm_pt, pixel_pt);
    }
  };

  // 计算消失点的像素坐标
  Eigen::Vector2d vp_x_pixel, vp_y_pixel, vp_z_pixel;
  normToPixel(vp_x_norm, vp_x_pixel);
  normToPixel(vp_y_norm, vp_y_pixel);
  normToPixel(vp_z_norm, vp_z_pixel);

  // 计算归一化平面上的直线与图像边界的交点
  auto intersectLineWithImageBorder =
      [&](const Eigen::Vector2d &vp_pixel,
          const Eigen::Vector2d &mid_pixel) -> Eigen::Vector2d {
    // 计算从消失点到中点的直线参数方程: P = vp_pixel + t * (mid_pixel -
    // vp_pixel) 我们需要找到t使得P在图像边界上，且0 <= t <=
    // 1（在vp_pixel和mid_pixel之间）

    double t = 1.0; // 默认使用中点
    Eigen::Vector2d intersection = mid_pixel;

    // 计算直线方向
    Eigen::Vector2d dir = mid_pixel - vp_pixel;
    ...
    // 左边界 x=0
    if (fabs(dir.x()) > 1e-6) {
      double t_left = (0 - vp_pixel.x()) / dir.x();
      if (t_left > 0 && t_left < t) {
        double y = vp_pixel.y() + t_left * dir.y();
        if (y >= 0 && y < img_height) {
          t = t_left;
          intersection = Eigen::Vector2d(0, y);
        }
      }
    }
    ...
    // 右边界 x=img_width-1
    if (fabs(dir.x()) > 1e-6) {
      double t_right = (img_width - 1 - vp_pixel.x()) / dir.x();
      if (t_right > 0 && t_right < t) {
        double y = vp_pixel.y() + t_right * dir.y();
        if (y >= 0 && y < img_height) {
          t = t_right;
          intersection = Eigen::Vector2d(img_width - 1, y);
        }
      }
    }
    ...
    // 上边界 y=0
    if (fabs(dir.y()) > 1e-6) {
      double t_top = (0 - vp_pixel.y()) / dir.y();
      if (t_top > 0 && t_top < t) {
        double x = vp_pixel.x() + t_top * dir.x();
        if (x >= 0 && x < img_width) {
          t = t_top;
          intersection = Eigen::Vector2d(x, 0);
        }
      }
    }
    ...
    // 下边界 y=img_height-1
    if (fabs(dir.y()) > 1e-6) {
      double t_bottom = (img_height - 1 - vp_pixel.y()) / dir.y();
      if (t_bottom > 0 && t_bottom < t) {
        double x = vp_pixel.x() + t_bottom * dir.x();
        if (x >= 0 && x < img_width) {
          t = t_bottom;
          intersection = Eigen::Vector2d(x, img_height - 1);
        }
      }
    }
  ...
  // 处理每条观测到的结构线
  for (const auto &line_entry : cur_lines) {
    int line_id = line_entry.first;
    const auto &line_data = line_entry.second.second;

    // 提取线段端点的归一化坐标
    Eigen::Vector2d pt_s_norm(line_data(0), line_data(1));
    Eigen::Vector2d pt_e_norm(line_data(2), line_data(3));
    Eigen::Vector2d pt_m_norm = (pt_s_norm + pt_e_norm) * 0.5;

    // 在struct_line_manager中查找线类型
    LineType line_type = OTHER;
    for (const auto &struct_line : struct_line_manager.struct_line_features) {
      if (struct_line.feature_id == line_id) {
        line_type = struct_line.line_type;
        break;
      }
    }
    ...
    // 根据线类型选择颜色和对应的消失点
    cv::Scalar color;
    Eigen::Vector2d vp_pixel;
    std::string vp_label;

    switch (line_type) {
    case HORIZON_X:
      color = cv::Scalar(0, 0, 255); // 红色 (BGR格式)
      vp_pixel = vp_x_pixel;
      vp_label = "VP-X";
      break;
    case HORIZON_Y:
    ...
    // 将归一化坐标转换到像素坐标
    Eigen::Vector2d pt_s_pixel, pt_e_pixel, pt_m_pixel;
    normToPixel(pt_s_norm, pt_s_pixel);
    normToPixel(pt_e_norm, pt_e_pixel);
    normToPixel(pt_m_norm, pt_m_pixel);

    cv::Point2f pt1(pt_s_pixel.x(), pt_s_pixel.y());
    cv::Point2f pt2(pt_e_pixel.x(), pt_e_pixel.y());
    cv::Point2f pt_m(pt_m_pixel.x(), pt_m_pixel.y());

    // 绘制观测线段
    cv::line(image, pt1, pt2, color, 2, cv::LINE_AA);

    // 判断消失点是否在图像内
    bool vp_in_image = (vp_pixel.x() >= 0 && vp_pixel.x() < img_width &&
                        vp_pixel.y() >= 0 && vp_pixel.y() < img_height);

    if (vp_in_image) {
      // 消失点在图像内，直接绘制从消失点到线段中点的连线
      cv::Point2f vp_pt(vp_pixel.x(), vp_pixel.y());
      cv::line(image, vp_pt, pt_m, color, 1, cv::LINE_AA);

      // 绘制消失点
      cv::circle(image, vp_pt, 1, color, -1);
      // cv::putText(image, vp_label, vp_pt + cv::Point2f(10, 10),
      //             cv::FONT_HERSHEY_SIMPLEX, 0.5, color, 1);
    } else {
      // 消失点在图像外，计算连线与图像边界的交点
      Eigen::Vector2d intersection_pixel =
          intersectLineWithImageBorder(vp_pixel, pt_m_pixel);
      cv::Point2f intersection_pt(intersection_pixel.x(),
                                  intersection_pixel.y());

      // 绘制从交点到线段中点的连线
      cv::line(image, intersection_pt, pt_m, color, 1, cv::LINE_AA);
    }
  }
