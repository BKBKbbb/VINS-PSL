...
FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img,
  ...
  */
  ...
  if (SHOW_TRACK) {
    // drawTrack(cur_img, rightImg, ids, cur_pts, cur_right_pts,
    // prevLeftPtsMap);
    onlyDrawPts(cur_img, cur_pts);
  }
...
void FeatureTracker::onlyDrawPts(const cv::Mat &img,
                                 vector<cv::Point2f> &cur_pts) {
  imTrack = img.clone();
  cv::cvtColor(imTrack, imTrack, cv::COLOR_GRAY2RGB);
  for (size_t j = 0; j < cur_pts.size(); j++) {
    cv::circle(imTrack, cur_pts[j], 2, cv::Scalar(0, 0, 255), 2);
  }
}
